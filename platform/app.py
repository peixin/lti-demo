...
DATABASE = os.environ.get(
    "DATABASE", os.path.join(os.path.dirname(__file__), "data/platform.db")
)
...
def migrate_db():
    ...
    with app.app_context():
        ...
        acols = {
            row[1] for row in db.execute("PRAGMA table_info(activities)").fetchall()
        }
        ...
        evcols = {
            row[1] for row in db.execute("PRAGMA table_info(score_events)").fetchall()
        }
...
def index():
    courses = (
        get_db()
        .execute("SELECT * FROM courses WHERE teacher_id=?", [session["user_id"]])
        .fetchall()
    )
    return render_template(
        "dashboard.html", courses=courses, username=session["username"]
    )
    ...
    if request.method == "POST":
        ...
        db.execute(
            ...
            [
                ...
                client_id,
                deployment_id,
                request.form["login_url"].strip(),
                request.form["redirect_uri"].strip(),
                request.form["jwks_url"].strip(),
                request.form["target_link_uri"].strip(),
            ],
        )
        db.commit()
...
def course_detail(course_id):
    ...
    activities = db.execute(
        ...
        "JOIN lti_tools t ON a.tool_id=t.id WHERE a.course_id=?",
        [course_id],
    ).fetchall()
    ...
    activities = [
        {
            **dict(a),
            "dl_custom": json.loads(a["deep_link_custom"])
            if a["deep_link_custom"]
            else None,
        }
        for a in activities
    ]
    ...
    if dl_configured.isdigit():
        dl_done_act = next(
            (a for a in activities if a["id"] == int(dl_configured)), None
        )
    return render_template(
        "course_detail.html",
        course=course,
...
def oidc_auth():
    ...
    db = get_db()
    tool = db.execute(
        "SELECT * FROM lti_tools WHERE client_id=?", [client_id]
    ).fetchone()
    if not tool:
    ...
    if hint.startswith("dl:"):
        return _sign_deep_linking(
            hint, client_id, tool, config, iss, login_hint, redirect_uri, state, nonce
        )
    if hint.startswith("review:"):
        return _sign_submission_review(
            hint, client_id, tool, config, iss, login_hint, redirect_uri, state, nonce
        )
...
def _sign_resource_link(
    activity_id, client_id, tool, config, iss, login_hint, redirect_uri, state, nonce
):
    db = get_db()
    activity = db.execute(
    ...
    custom = (
        json.loads(activity["deep_link_custom"])
        if activity["deep_link_custom"]
        else None
    )
    ...
    return render_template(
        "oidc_response.html", redirect_uri=redirect_uri, id_token=id_token, state=state
    )


def _sign_deep_linking(
    hint, client_id, tool, config, iss, login_hint, redirect_uri, state, nonce
):
    db = get_db()
    activity_id = int(hint.split(":", 1)[1])
...
def _sign_submission_review(
    ...
    target_link_uri = activity["deep_link_uri"] or tool["target_link_uri"]
    custom = (
        json.loads(activity["deep_link_custom"]) if activity["deep_link_custom"] else {}
    )
    if event_id:
    ...
    id_token = make_id_token(
        ...
        return_url=iss + f"/courses/{activity['course_id']}",
        roles=[ROLE_INSTRUCTOR],
        for_user={"user_id": str(target_user_id), "name": target["username"]},
...
def lti_dl_response():
    ...
    iss = request.host_url.rstrip("/")
    try:
        claims = verify_dl_response(
            token, tool["jwks_url"], expected_aud=iss, expected_iss=client_id
        )
    except Exception as e:
...
def ags_scores(lineitem_id):
    ...
    score_given = (
        float(data["scoreGiven"]) if data.get("scoreGiven") is not None else None
    )
    score_maximum = (
        float(data["scoreMaximum"]) if data.get("scoreMaximum") is not None else None
    )
...
def nrps_memberships(course_id):
    ...
    memberships_url = (
        request.host_url.rstrip("/") + f"/lti/nrps/{course_id}/memberships"
    )
    return jsonify(
        {
            "id": memberships_url,
